...
const CheckIcon = () => (
  <svg className="inline-block mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="green" width="18" height="18">
...
const AlertCard: React.FC<{ alert: AlertType }> = ({ alert }) => {
  ...
  return (
    <div className={`max-w-3xl m-auto rounded-xl border-l-4 ${borderColors[alert.severity]} bg-white/80 dark:bg-zinc-900 shadow-md mb-4 transition-all hover:shadow-lg hover:scale-[1.02]`}>
      <button
      ...
      >
        <div className="flex items-center">
          <span className={`inline-block w-3 h-3 rounded-full mr-2 ${
            ...
            alert.severity === "high" ? "bg-orange-400 dark:bg-orange-600" :
            alert.severity === "medium" ? "bg-yellow-300 dark:bg-yellow-600" :
        ...
        <span className={`px-3 py-1 rounded-full text-xs font-semibold text-white ${severityColors[alert.severity].light} dark:${severityColors[alert.severity].dark}`}>
      ...
      {open && (
        <div className="p-4 pt-0 text-gray-700 dark:text-gray-200 text-sm space-y-3 border-t border-gray-200 dark:border-zinc-700">
          ...
          <div>
            <strong>Time:</strong>
            <div className="ml-2">{new Date(alert.timestamp).toLocaleString()}</div>
          </div>

          {alert.predictedAt && (
            <div>
              <strong>Predicted Time:</strong>
              <div className="ml-2">{new Date(alert.predictedAt).toLocaleString()}</div>
            </div>
          )}
          ...
          {alert.sensorData && (
            <div>
              ...
              <ul className="mt-1 pl-5 list-disc space-y-1">
                {alert.sensorData.bp && <li>Blood Pressure: {alert.sensorData.bp.systolic}/{alert.sensorData.bp.diastolic} mmHg</li>}
                ...
                {alert.sensorData.fallDetected !== undefined && <li>Fall Detected: {alert.sensorData.fallDetected ? "Yes" : "No"}</li>}
              </ul>
            </div>
          )}
          ...
          <div>
            ...
            <div className="ml-2">
              {alert.isAcknowledged ? (
                <span className="text-green-600 dark:text-green-400 inline-flex items-center"><CheckIcon /> Acknowledged</span>
              ) : (
          ...
          {alert.isAcknowledged && (
            <>
              ...
              <div>
                ...
                <div className="ml-2">{alert.acknowledgedAt ? new Date(alert.acknowledgedAt).toLocaleString() : "-"}</div>
              </div>
            </>
          )}
...
const AlertList: React.FC = () => {
  ...
  return (
    <div className=" mx-auto py-10 px-4 sm:px-6 bg-blue-50 dark:bg-zinc-900 min-h-screen transition-colors duration-300">
      <h1 className="text-center mb-8 font-bold text-2xl text-blue-800 dark:text-blue-400">Health Alerts Dashboard</h1>
