...
import { ArrowUpIcon, StopIcon, SummarizeIcon } from "../chat/icons"
import { artifactDefinitions, type ArtifactKind } from '../chat/artifact'
import type { ArtifactToolbarItem } from '../chat/create-artifact';
...
type ToolProps = {
  ...
  isAnimating: boolean;
  sendMessage: UseChatHelpers<ChatMessage>;
  onClick: ({
    sendMessage,
  }: {
    sendMessage: UseChatHelpers<ChatMessage>;
        ...
        {isToolbarVisible &&
          secondaryTools.map((secondaryTool) => (
            <Tool
              ...
              onClick={({ sendMessage }) =>
                secondaryTool.onClick({
                  sendMessage: (message) =>
                    sendMessage.append({
                      ...message,
                      // Ensure role is typed correctly and parts exist
                      id: message.id ?? undefined,
                      role: message.role as 'user' | 'assistant' | 'system',
                      parts: 'parts' in message ? (message as any).parts ?? [] : [],
                    }),
                })
              }
            />
